...
@section('title', 'Riwayat Saya - Sistem Perpustakaan')

@section('content')
    <div class="page-header">
        <div>
            <h1>Riwayat Peminjaman Saya</h1>
            <p class="subtitle">Catatan historis seluruh buku yang pernah Anda pinjam dan kembalikan.</p>
        </div>
        <a class="button button-primary" href="{{ route('peminjam.books.index') }}">📚 Pinjam Buku Lain</a>
    </div>

    <!-- Statistik Ringkas Riwayat Pribadi -->
    <div class="stats-grid">
        <div class="stat-card">
            <div class="stat-icon" style="background: #eff6ff; color: #2563eb;">📋</div>
            <div class="stat-info">
                <span class="stat-label">Total Riwayat Transaksi</span>
                <strong class="stat-value">{{ $totalTransactions }} kali</strong>
            </div>
        </div>
        ...
        <div class="stat-card">
            ...
            <div class="stat-info">
                <span class="stat-label">Total Denda Yang Pernah Dibayar</span>
    ...
    <div class="filter-card">
        <form action="{{ route('peminjam.history') }}" method="GET" class="filter-form">
            ...
            <div class="filter-buttons">
                ...
                @if(request('search') || request('status'))
                    <a href="{{ route('peminjam.history') }}" class="button button-secondary">Reset</a>
                @endif
            </div>
        </form>
    </div>

    <!-- Tabel Riwayat Transaksi Peminjam -->
    <div class="table-wrap">
        @if ($transactions->isEmpty())
            <div class="empty-state">
                <div class="empty-icon">📋</div>
                @if (request('search') || request('status'))
                    <h3>Tidak ada riwayat transaksi yang cocok.</h3>
                    <p class="muted">Coba ubah kata kunci atau filter status Anda.</p>
                    <a href="{{ route('peminjam.history') }}" class="button button-secondary" style="margin-top: 12px;">Reset Filter</a>
                @else
                    <h3>Belum ada catatan riwayat peminjaman.</h3>
                    <p class="muted">Setelah buku dikembalikan, riwayat lengkap akan tercatat di halaman ini.</p>
                @endif
            </div>
        @else
            <table>
