...
@section('title', 'Kategori Buku - Sistem Manajemen Perpustakaan')
...
@section('content')
    <div class="page-header">
        ...
        <a class="button button-primary" href="{{ route('categories.create') }}">+ Tambah Kategori</a>
    </div>
    ...
    <div class="filter-card">
        <form action="{{ route('categories.index') }}" method="GET" class="filter-form">
            ...
            <div class="filter-buttons">
                ...
                @if(request('search'))
                    <a href="{{ route('categories.index') }}" class="button button-secondary">Reset</a>
                @endif
            </div>
        </form>
    </div>
        ...
        @if ($categories->isEmpty())
            <div class="empty-state">
                ...
                @if (request('search'))
                    ...
                    <a href="{{ route('categories.index') }}" class="button button-secondary" style="margin-top: 12px;">Reset Pencarian</a>
                @else
                    <h3>Belum ada kategori buku.</h3>
                    <p class="muted">Mulai dengan menambahkan kategori baru ke sistem Anda.</p>
                    <a href="{{ route('categories.create') }}" class="button button-primary" style="margin-top: 12px;">+ Tambah Kategori Pertama</a>
                @endif
            </div>
        @else
            <table>
                ...
                <tbody>
                    @foreach ($categories as $index => $category)
                        <tr>
                            ...
                            <td>
                                <div class="actions">
                                    <a class="button button-edit" href="{{ route('categories.edit', $category) }}">Edit</a>
