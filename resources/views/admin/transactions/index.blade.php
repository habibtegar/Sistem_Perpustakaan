...
@section('title', 'Riwayat Transaksi - Administrator')

@section('content')
    <div class="page-header">
        <div>
            <h1>Riwayat Transaksi Perpustakaan</h1>
            <p class="subtitle">Seluruh log riwayat peminjaman, persetujuan, penolakan, dan pengembalian buku.</p>
        </div>
        ...
        <form action="{{ route('admin.transactions.index') }}" method="GET" class="filter-form">
            ...
            <div class="filter-buttons">
                ...
                @if(request('search') || request('status'))
                    <a href="{{ route('admin.transactions.index') }}" class="button button-secondary">Reset</a>
                @endif
            </div>
        </form>
    </div>
        ...
        @if ($transactions->isEmpty())
            <div class="empty-state">
                ...
                @if (request('search') || request('status'))
                    ...
                    <a href="{{ route('admin.transactions.index') }}" class="button button-secondary" style="margin-top: 12px;">Reset Filter</a>
                @else
