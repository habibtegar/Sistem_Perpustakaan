...
@section('title', 'Kelola Pengembalian - Administrator')

@section('content')
    <div class="page-header">
        <div>
            <h1>Pengembalian Buku</h1>
            <p class="subtitle">Proses pengembalian buku, penambahan stok otomatis, dan perhitungan denda keterlambatan.</p>
        </div>
        <div class="page-header-actions">
            ...
            <a href="{{ route('admin.settings.index') }}" class="button button-secondary button-sm">⚙️ Ubah Tarif</a>
        </div>
    </div>

    <!-- Filter & Search Bar -->
    <div class="filter-card">
        <form action="{{ route('admin.returns.index') }}" method="GET" class="filter-form">
            ...
            <div class="filter-buttons">
                ...
                @if(request('search'))
                    <a href="{{ route('admin.returns.index') }}" class="button button-secondary">Reset</a>
                @endif
            </div>
        </form>
    </div>
        ...
        @if ($borrowings->isEmpty())
            <div class="empty-state">
                ...
                @if (request('search'))
                    ...
                    <a href="{{ route('admin.returns.index') }}" class="button button-secondary" style="margin-top: 12px;">Reset</a>
                @else
                ...
                <tbody>
                    @foreach ($borrowings as $index => $trx)
                        ...
                        <tr style="{{ $isOverdue ? 'background-color: #fffafa;' : '' }}">
                            ...
                            <td>
                                <form action="{{ route('admin.returns.process', $trx) }}" method="POST" onsubmit="return confirm('Konfirmasi pengembalian buku {{ addslashes($trx->book->title ?? '') }} dari {{ addslashes($trx->member->name ?? '') }}? {{ $fine > 0 ? '\n\nTotal Denda: Rp ' . number_format($fine, 0, ',', '.') : '' }}')">
                                    @csrf
