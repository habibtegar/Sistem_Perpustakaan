...
@section('content')
    <div class="page-header">
        <div>
            ...
            <p class="subtitle">Informasi profil dan riwayat peminjaman buku anggota.</p>
        </div>
        <div class="page-header-actions">
            <a class="button button-secondary" href="{{ route('members.index') }}">&larr; Kembali</a>
            <a class="button button-edit" href="{{ route('members.edit', $member) }}">Edit Data</a>
        </div>
    </div>
    ...
    <div class="detail-card">
        ...
        @if ($member->transactions->isEmpty())
            <div class="empty-state" style="padding: 24px 0; background: #f8fafc; border-radius: 8px;">
                ...
                <a href="{{ route('borrowings.create') }}" class="button button-primary button-sm" style="margin-top: 10px;">+ Pinjamkan Buku</a>
            </div>
        @else
                    ...
                    <tbody>
                        @foreach ($member->transactions as $trx)
                            <tr>
                                ...
                                <td>
                                    @if($trx->fine_amount > 0 || $trx->current_fine > 0)
                                        <span style="color: #dc2626; font-weight: 600;">
                                            Rp {{ number_format($trx->status === 'Dikembalikan' ? $trx->fine_amount : $trx->current_fine, 0, ',', '.') }}
                                        </span>
                                    @else
