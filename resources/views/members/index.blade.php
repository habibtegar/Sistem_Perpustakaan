...
@section('title', 'Daftar Anggota - Sistem Manajemen Perpustakaan')

@section('content')
    <div class="page-header">
        <div>
            <h1>Manajemen Anggota</h1>
            <p class="subtitle">Kelola data keanggotaan siswa/pembaca perpustakaan.</p>
        </div>
        <a class="button button-primary" href="{{ route('members.create') }}">+ Tambah Anggota</a>
    </div>
    ...
    <div class="filter-card">
        <form action="{{ route('members.index') }}" method="GET" class="filter-form">
            <div class="filter-group filter-search">
                ...
                <input
                    ...
                    type="text"
                    name="search"
                    placeholder="Cari nama, ID anggota, kelas, no HP..."
            ...
            <div class="filter-buttons">
                ...
                @if(request('search') || request('status'))
                    <a href="{{ route('members.index') }}" class="button button-secondary">Reset</a>
                @endif
            </div>
        </form>
    </div>
        ...
        @if ($members->isEmpty())
            <div class="empty-state">
                ...
                @if (request('search') || request('status'))
                    ...
                    <a href="{{ route('members.index') }}" class="button button-secondary" style="margin-top: 12px;">Reset Filter</a>
                @else
                    <h3>Belum ada data anggota.</h3>
                    <p class="muted">Daftarkan anggota perpustakaan untuk mulai mencatat peminjaman.</p>
                    <a href="{{ route('members.create') }}" class="button button-primary" style="margin-top: 12px;">+ Tambah Anggota Pertama</a>
                @endif
            </div>
        @else
            <table>
                ...
                <tbody>
                    @foreach ($members as $index => $member)
                        <tr>
                            ...
                            <td>
                                <div class="actions">
                                    <a class="button button-info" href="{{ route('members.show', $member) }}">Detail</a>
                                    <a class="button button-edit" href="{{ route('members.edit', $member) }}">Edit</a>
                                    <form action="{{ route('members.destroy', $member) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus anggota ini?')">
                                        @csrf
                                        @method('DELETE')
