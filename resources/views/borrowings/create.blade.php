...
@section('title', 'Form Peminjaman Buku - Sistem Manajemen Perpustakaan')

@section('content')
    <div class="page-header">
        <div>
            <h1>Form Peminjaman Buku</h1>
            <p class="subtitle">Catat peminjaman buku oleh anggota perpustakaan.</p>
        </div>
        <a class="button button-secondary" href="{{ route('borrowings.index') }}">&larr; Kembali</a>
    </div>
    ...
    <div class="form-card">
        ...
        <form action="{{ route('borrowings.store') }}" method="POST" id="borrowForm">
            ...
            <div class="form-group">
                ...
                @if($members->isEmpty())
                    <p class="error">Belum ada anggota aktif terdaftar. Silakan <a href="{{ route('members.create') }}" style="text-decoration: underline;">tambah anggota</a> terlebih dahulu.</p>
                @else
            ...
            <div class="form-group">
                ...
                @if($allBooks->isEmpty())
                    <p class="error">Belum ada buku di perpustakaan. Silakan <a href="{{ route('books.create') }}" style="text-decoration: underline;">tambah buku</a> terlebih dahulu.</p>
                @else
            ...
            <div class="form-grid">
                ...
                <div class="form-group">
                    ...
                    <input
                        ...
                        required
                    >
                    <p class="helper-text">Standar peminjaman adalah {{ config('library.default_loan_days', 7) }} hari.</p>
            ...
            <div class="form-group">
                ...
                <textarea id="notes" name="notes" placeholder="Kondisi buku saat dipinjam atau catatan lainnya...">{{ old('notes') }}</textarea>
                @error('notes') <p class="error">{{ $message }}</p> @enderror
            </div>

            <div class="form-actions">
                <button type="submit" id="btnSubmit" class="button button-primary">📤 Pinjam Buku</button>
                <a href="{{ route('borrowings.index') }}" class="button button-secondary">Batal</a>
            </div>
        </form>
    </div>
    ...
    <script>
        ...
        function updateDueDate(borrowDateStr) {
            ...
            const loanDays = {{ (int) config('library.default_loan_days', 7) }};
