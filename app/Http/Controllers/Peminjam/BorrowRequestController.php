...
class BorrowRequestController extends Controller
{
    public function store(Request $request, Book $book): RedirectResponse
    {
        ...
        $request->validate([
            ...
            'due_date.date' => 'Format tanggal rencana pengembalian tidak valid.',
            'due_date.after_or_equal' => 'Tanggal rencana pengembalian minimal hari ini atau setelahnya.',
        ...
        if (empty($notes)) {
            $notes = 'Pengajuan peminjaman oleh ' . $member->name . ' (Rencana kembali: ' . $dueDate->format('d/m/Y') . ')';
        }
        ...
        return redirect()->route('peminjam.my-borrowings')->with('success', "Permohonan peminjaman buku '{$book->title}' berhasil diajukan dengan rencana kembali pada {$dueDate->format('d/m/Y')}! Menunggu persetujuan Admin.");
    }
}
