...
class ReturnController extends Controller
{
    public function index(Request $request): View
    {
        ...
        $fineRate = (int) Setting::get('fine_per_day', config('library.fine_per_day', 1000));

        return view('admin.returns.index', compact('borrowings', 'overdueCount', 'fineRate'));
    }

    public function process(Request $request, Transaction $transaction): RedirectResponse
    {
        if ($transaction->status !== 'Dipinjam') {
            return redirect()->route('admin.returns.index')->with('error', 'Status transaksi saat ini bukan Dipinjam.');
        }
